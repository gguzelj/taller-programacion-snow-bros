...
void Contacto::BeginContact(b2Contact* contact) {
	...
	/*
	 ...
	 && fixtureAux->GetBody()->GetType() == b2_staticBody) {
	 for (auto personaje = characters->begin();
	 personaje != characters->end(); personaje++) {
...
void Contacto::EndContact(b2Contact* contact) {
	...
	/*
	 ...
	 if ((*personaje)->getb2Body() == fixture->GetBody()) {
	 (*personaje)->cantidadDeContactosActuales--;
	 if ((*personaje)->cantidadDeContactosActuales
	 == 0&& (*personaje)->state->getCode() != JUMPING)
